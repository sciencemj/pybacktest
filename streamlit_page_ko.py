...
def show_korean_page():
    ...
    with st.sidebar:
        # 1. 파일 업로더
        uploaded_file = st.file_uploader("JSON 설정 파일 불러오기", type=["json"], key="kr_upload")
        
        if uploaded_file is not None:
            # 파일이 업로드되면 버튼을 표시하여 의도치 않은 덮어쓰기 방지
            if st.button("데이터 적용하기", type="primary", use_container_width=True, key="kr_apply"):
                try:
    ...
    with tab1:
        ...
        with left_col:
            ...
            # 메인 Ticker 입력
            main_ticker = st.text_input("메인 Ticker (편집할 종목명 입력)", value="AAPL", key="kr_main_ticker").upper()
            ...
            if not main_ticker:
                ...
                if st.button(btn_label, use_container_width=True, key=f"kr_save_{main_ticker}"):
        ...
        with col21:
            ...
            if 'backtest' in st.session_state and st.session_state['backtest']:
                with st.container(border=True):
                    ...
                    date = st.slider('date', start, end, end, label_visibility= "hidden", key="kr_slider")
