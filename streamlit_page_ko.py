...
def show_korean_page():
    ...
    with st.sidebar:
        ...
        uploaded_file = st.file_uploader(
            "JSON 설정 파일 불러오기", type=["json"], key="kr_upload"
        )
        ...
        # 2. 초기화 버튼
        if st.button("🗑️ 전체 초기화", use_container_width=True, key="kr_reset"):
    ...
    def input_strategy_details(
        ...
        with col1:
            target_ticker = st.text_input(
                "대상 Ticker", value=def_ticker, key=f"{key_prefix}_ticker"
            )
            ...
            by_agg = c1.selectbox(
                "집계 방식", opts_agg, index=idx_agg, key=f"{key_prefix}_by_agg"
            )
            by_field = c2.selectbox(
                "필드", opts_field, index=idx_field, key=f"{key_prefix}_by_field"
            )
        ...
        with col2:
            ...
            use_period = st.checkbox(
                "기간 설정 사용", value=def_use_period, key=f"{key_prefix}_use_period"
            )
            if use_period:
        ...
        with col3:
            ...
            crit_type = c3.selectbox(
                "조건 타입", opts_crit, index=idx_crit, key=f"{key_prefix}_crit_type"
            )
        ...
        with col4:
            ...
            qty_type = c5.selectbox(
                "단위", opts_qty, index=idx_qty, key=f"{key_prefix}_qty_type"
            )
            qty_val = c6.number_input(
                "수량 값", value=def_qty_val, step=1.0, key=f"{key_prefix}_qty_val"
            )
    ...
    with tab1:
        ...
        with left_col:
            ...
            main_ticker = st.text_input(
                "메인 Ticker (편집할 종목명 입력)", value="AAPL", key="kr_main_ticker"
            ).upper()
            ...
            if not main_ticker:
                ...
                if current_data:
                    st.info(
                        f"💾 기존에 저장된 **[{main_ticker}]** 데이터를 불러왔습니다."
                    )
                else:
                ...
                if st.button(
                    btn_label, use_container_width=True, key=f"kr_save_{main_ticker}"
                ):
        ...
        with right_col:
            ...
            if st.session_state["strategies"]:
                ...
                st.info(
                    "데이터가 비어있습니다. 왼쪽에서 추가하거나 JSON 파일을 업로드하세요."
                )
    with tab2:
        st.subheader("백테스트")
        col21, col22 = st.columns([0.5, 1])
        with col21:
            with st.form("backtest_form_kr"):
                col31, col32 = st.columns([1, 1])
                start = col31.date_input("시작일", value=pd.to_datetime("2023-01-01"))
                end = col32.date_input("종료일")
                initial_cash = st.number_input("초기 자금", value=10000)
                run_button = st.form_submit_button(
                    "백테스트 시작!", use_container_width=True
                )
                if run_button:
                    stocks = []
        ...
        with col22:
            if "backtest" in st.session_state and st.session_state["backtest"]:
                ...
                st.markdown(f"## 최종 이익률: {profit_rate:.3f}")
